...
- [x] 使用启用 Bypass 2FA 的 granular token 发布 `@ssbun/sharkspace@1.13.0` 并确认 latest。
...
- 新 granular token 已启用 Bypass 2FA，`npm whoami` 返回 `ssbun`。
...
- 旧 token 的正式 publish 返回 `EOTP`；换用启用 Bypass 2FA 的 token 后发布成功，registry `latest` 为 `1.13.0`。
...
- `npm pack --dry-run --json` 与 `npm publish --dry-run --access public --json` 通过，tarball 仅含 README、install.js、package.json；正式 publish 后 registry 回读版本与 `latest` 均为 `1.13.0`。
...
- 修复后的 tag workflow 全部通过；GitHub Release 与两个 asset 已完成远端下载和 checksum 验证，npm package 的版本、tarball integrity 和 `latest` 已从 registry 回读确认。
